...
export function createFujuClient(opts: FujuClientOptions): FujuClient {
  ...
  const withJson = (body: unknown): { body: BodyInit | null; headers: Record<string, string> } => {
  ...
  return {
    async get<T>(path: string, o?: RequestOpts): Promise<T> {
      const res = await doRequest('GET', path, { signal: o?.signal, query: o?.query });
    ...
    async del<T = void>(path: string, o?: RequestOpts): Promise<T> {
      const res = await doRequest('DELETE', path, { signal: o?.signal, query: o?.query });
      await throwIfError(res);
      return parseBody<T>(res);
    },
    async postForm<T>(path: string, form: FormData, o?: RequestOpts): Promise<T> {
      const res = await doRequest('POST', path, {
        body: form,
        signal: o?.signal,
        query: o?.query,
