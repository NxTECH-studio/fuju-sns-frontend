...
export function UserProfileRoute() {
  ...
  if (profile.error) return <p style={{ color: '#d33' }}>エラー: {profile.error}</p>;
  ...
  const handleFollowToggle = async () => {
    try {
      ...
      toast.show(e instanceof Error ? e.message : 'フォローに失敗しました', 'error');
    }
  };

  const followAction =
    !isSelf && status === 'authenticated' ? (
      <FollowButton
